...
写教程/营销文章时，"看得见的成果"和"专业的原理图"是质量分水岭。`templates/` 提供 4 个即用模板，复制改文案即可，配合 `screenshot.py` / `html_to_gif.py` 产出。**定位：配图默认用 gpt-image-2 生图（见 article-writing-pipeline）；HTML 模板是兜底——仅当中文信息图反复乱码、需精确控制布局、或要做动态 GIF 时才用。**

...
凭证只读 config.json（不读环境变量，避免系统残留失效变量干扰）。
HTTP 走 curl（绕过 llm-link.top 前置 WAF 的 TLS 指纹拦截）。
"""
            ...
            if os.path.isfile(cfgp):
                ...
                return {
                    "api_key": raw.get("api_key") or raw.get("key")
                               or raw.get("llm_link_api_key") or "",
                    "base_url": (raw.get("base_url") or raw.get("url")
                                 or raw.get("llm_link_base_url")
                                 or "https://www.llm-link.top"),
                    "model": (raw.get("model") or raw.get("llm_link_model")
                              or "gpt-image-2"),
                    "_path": cfgp,
                }
...
def main():
    ...
    api_key = cfg["api_key"]
    if not api_key:
        print("未找到 api_key: 请在技能包根目录配置 config.json(参考 config.example.json)",
              file=sys.stderr)
        sys.exit(3)

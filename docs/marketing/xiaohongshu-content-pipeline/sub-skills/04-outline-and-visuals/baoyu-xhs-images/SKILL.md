...
**主力生图：本地脚本 `scripts/generate_image.py`（gpt-image-2 via LLM-Link，零依赖）**

1. 把每张图的完整提示词写入笔记目录下的 `prompts.md`，按 section 分节。脚本用正则
   `## #<section> ... 完整提示词:` + ```` ``` ```` 代码块提取，格式必须如下：

   ```markdown
   ## #cover 封面
   完整提示词:
   ```
   <这里写封面的完整英文/中文提示词，含 style/layout/文案>
   ```
...
2. 凭证：首次使用复制本子技能下 `config.example.json` 为 `config.json`，填 `key`
   （可复用 wechat 同款 key）。脚本自动向上查找 `config.json`，**只读配置文件、
   不读环境变量**，避免系统残留的失效环境变量干扰。`config.json` 已被 gitignore。

3. 逐张生成（小红书竖版 3:4，`--size 1080x1440`；封面也可 `--size 1080x1440`）：

   ```bash
   cd <baoyu-xhs-images 子技能目录>
   python scripts/generate_image.py \
     --prompt-file <笔记目录>/prompts.md --section cover \
     --output <笔记目录>/images/01-cover.png --size 1080x1440
   python scripts/generate_image.py \
     --prompt-file <笔记目录>/prompts.md --section content-1 \
     --output <笔记目录>/images/02-content-1.png --size 1080x1440
   ```

4. 每张生成后报告进度；失败自动重试 3 次（脚本内置）。中文反复乱码时，退回
   `card-xiaohongshu`（HTML 卡片截图）作为兜底。

**输出目录**：图片统一落在笔记目录的 `images/` 下，提示词留存在同目录 `prompts.md`，
便于历史复现与单张重生（重生即用相同 section 重跑脚本）。

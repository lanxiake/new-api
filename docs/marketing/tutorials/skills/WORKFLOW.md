...
实测 gpt-image-2 **中文已基本可用**。用 chat completions 接口生成（见下方脚本模式）。
- 封面：专业科技/科幻/商务风，每篇换主色调+视觉隐喻，无卡通萌系
- 信息图：PPT 风，SVG/line icon（**不用 Emoji**），流程图/矩阵/卡片，霓虹描边玻璃质感
- **每张必人工核验中文**，错了或 504 就重试（3-5次）

生成脚本模式（内联提示词，避开文件路径坑）：
```python
import json,urllib.request,re,time
prompt='''<你的图片提示词，务必写明:清晰准确的简体中文、不要乱码假字、不要Emoji、用line icon、深蓝黑科技底>'''
key='<LLM-Link key>'
ua='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36'
payload=json.dumps({'model':'gpt-image-2','messages':[{'role':'user','content':prompt}]}).encode()
for att in range(5):
    try:
        req=urllib.request.Request('https://www.llm-link.top/v1/chat/completions',data=payload,
            headers={'Authorization':'Bearer '+key,'Content-Type':'application/json','User-Agent':ua},method='POST')
        c=json.loads(urllib.request.urlopen(req,timeout=600).read())['choices'][0]['message']['content']
        m=re.search(r'!\[[^\]]*\]\((https?://[^)\s]+)\)',c) or re.search(r'https?://\S+\.(png|jpg|jpeg|webp)',c)
        url=m.group(1) if m and m.lastindex else m.group(0)
        img=urllib.request.urlopen(urllib.request.Request(url,headers={'User-Agent':ua}),timeout=120).read()
        open('images/xx.png','wb').write(img); print('OK'); break
    except Exception as e: print('retry',e); time.sleep(5)
```

### 4b. 信息图 → HTML 兜底（要求文字 100% 精确时）
...
`articles/` 下三篇完整成品（认知篇 01-03），含 final.md / publish.md / 全部配图，是本套流程的活样板：
- `20260529-what-is-ai-coding/` 第01篇
- `20260601-requirement-skill/` 第02篇
- `20260605-be-product-manager/` 第03篇

新写文章时，照着任一篇的结构和配图套路复制即可。

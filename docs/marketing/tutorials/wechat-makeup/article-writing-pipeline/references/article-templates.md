...
>
> **分层视觉策略**（实测沉淀，按美观度优先级排）：
> - **封面图**：**专业科技/科幻/商务风**（深蓝科技底 + 数据流/神经网络/光效，人物用几何剪影或抽象全息，绝不卡通萌系）。用 gpt-image-2 生成。**每篇封面要有差异**——换主色调、换核心视觉隐喻。
> - **原理/信息图（首选 gpt-image-2 PPT 风）**：实测 gpt-image-2 现在**中文渲染基本可用**，能出 PPT 级精致信息图（霓虹描边、玻璃质感、SVG 风线性图标、流程箭头、四象限），美观度远超手工 HTML。**但文字不稳定，每张必须人工核验中文，错了重生成**（成功率：文字少/结构清晰的高，文字密集的低）。提示词务必写明"清晰准确的简体中文、不要乱码假字、不要 Emoji、用 line icon"。
> - **原理/信息图（备选 HTML 截图）**：当 gpt 反复出中文乱码、或要求文字 100% 精确可控时，退回 HTML 精确绘制 + CloakBrowser 截图（见 cloakbrowser-scraper 的 templates/，含 dark-infographic 黑底高级模板）。可控但美观度略逊 gpt。
> - **对比/演示图**：用 HTML+JS 做动态对比 GIF（如"模糊 vs 清晰需求"双路对话同屏），动态演进代入感最强，GIF 这类 gpt 做不了。
> - **成果/操作图**：用 CloakBrowser 截真实运行结果或终端，绝不用 AI 编。
>
> **信息设计 ≠ 刷黑底**（重要教训）：图高级不是因为底色黑，而是因为①信息分层（主次大小分明：核心词最大→说明次之→例子最小）②结构表意（用布局本身传递信息，如流程图表"步骤"、矩阵表"维度"）③视觉引导（箭头/编号/色块引导浏览顺序）④一图一核心。先想清楚"这张图要让读者一眼get什么"，再选图形。
>
> 关键教训：
> 1. **gpt-image-2 中文已基本可用但不稳定**——能用，但每张必核验、备好重试（504 也常见，多试几次）。要求文字 100% 精确时用 HTML 兜底。
> 2. 封面**早期试过卡通萌系，被否**——目标读者是想做事的成年人，要专业科技感。
> 3. **不用 Emoji，用 SVG/line icon**——Emoji 显廉价，线性图标显专业。

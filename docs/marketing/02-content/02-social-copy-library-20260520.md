...
> 适用场景：评论区回复、社区帖子、即刻/小红书/V2EX 短贴、Discord/QQ 群、Reddit 评论等。
> 邀请链接：https://www.llm-link.top/register?aff=3Us3 | QQ 群：1102925294

---

## 一、评论区回复型（针对"Claude Code 好贵"/"API 怎么搞"等话题）

**[1] 轻回复 · 引导咨询**

> 国内用 Claude Code 的话，可以考虑 API 中转，我现在用 LLM-Link，邀请制的，claude-opus-4-7 输入才 ¥4/1M，比官方便宜很多，需要的话可以私我拿邀请链接
...
> 用中转站最怕两件事：模型掺水 + 跑路。
> 我试过几家，LLM-Link 目前最稳——cc 分组的 Claude 是真实的，Streaming 没问题，邀请制控制用户规模，高峰期也扛得住。
> 有兴趣可以用这个链接注册试试：https://www.llm-link.top/register?aff=3Us3

---

## 二、主动发帖型（适合 V2EX / 即刻 / NodeSeek）
...
> 推荐一个 Claude Code 国内用的中转站：LLM-Link
>
...
> 提醒一下用 Claude Code 中转站的朋友：
>
> ① 倍率低得离谱的（0.2x Opus）基本是国产模型冒充，别被骗
> ② Streaming 有 bug 的别用，Claude Code 严重依赖流式输出
> ③ 充太多容易跑路血亏，建议每次充 ¥100-200
>
> 我现在稳定在用 LLM-Link，邀请制，目前没碰到上面这些问题。邀请链接：https://www.llm-link.top/register?aff=3Us3

---

## 三、小红书 / 即刻风格（短平快，带 emoji 可自行加）
...
## 六、B站 / 视频评论区型（针对 Claude Code 教程视频下）
...
> 补充一个：配置 `ANTHROPIC_BASE_URL` 记得不要在末尾加斜杠，会报 ECONNREFUSED。另外用 LLM-Link 的 cc-sale 分组，比视频里说的直连要省不少，有邀请链接的话 https://www.llm-link.top/register?aff=3Us3

---

## 七、知乎回答型（针对"Claude Code 怎么在国内用""中转站哪家靠谱"等问题）

**[18] 知乎长回答型 · 核心段落**

> 国内用 Claude Code 有几个绕不开的问题：网络不稳定、Visa 卡支付、配额不够用。中转站能一次性解决这三个。
>
> 但中转站坑也多：模型掺水的（拿国产模型冒充 Opus）、Streaming 有 bug 的、直接跑路的，我都遇到过。
>
...
> 用 LLM-Link，邀请制的，模型不掺水，cc-sale 分组 claude-opus-4-7 现在折后 ¥2/1M 输入，支持 Prompt Caching，稳定用了好几个月没出过问题。
>
> 链接：https://www.llm-link.top/register?aff=3Us3

---

## 八、GitHub / SegmentFault Issues 型（技术问题回复中自然带出）

**[20] 技术帖回复 · 解决问题顺带推荐**

> 你报 `401 Unauthorized` 大概率是 apiKey 没配对，或者分组选错了。Claude Code 必须用 `cc` 或 `cc-sale` 分组，不能用 `default` 分组的 key。
>
> 我用的是 LLM-Link（https://www.llm-link.top），配置方式：
> ```bash
> claude config set apiKey sk-你的cc-sale令牌
> claude config set baseUrl https://www.llm-link.top/v1
> ```
> 邀请制平台，model 是真实 Claude，Streaming 正常，价格也比官方便宜很多。
...
> 中转站用了这么多家，第一次觉得"邀请制"是个优点而不是门槛
>
> 开放注册的总被薅羊毛打烂，邀请制控制了用户质量，渠道稳定性反而更好
>
> LLM-Link 用了半年，高峰期没出过 502，模型没掺水，就这两点已经比大多数同类强了
> https://www.llm-link.top/register?aff=3Us3

---

## 十、AIDA 结构型（适合独立博客 / 公众号段落嵌入）

**[25] 完整 AIDA 结构广告段**

> **还在为 Claude Code 账单发愁？**
>
> 官方 Max 20x 订阅 $200/月，折合人民币接近 ¥1,500，重度用户还不够用，月底账单经常破 ¥2,000。
>
...
> 上个月有个朋友问我，为什么我用 Claude Code 每月才花两三百，他要花一千多。
>
> 我就发给他一个链接。
>
...
> 接外包用 Claude Code 辅助开发，成本算下来 LLM-Link cc-sale 分组 claude-opus-4-7 折后 ¥2/1M 输入，一个中等项目用个 20M tokens 也就 ¥40，相比节省的开发时间完全可以忽略。
>
> 邀请制的，需要链接的私我或直接：https://www.llm-link.top/register?aff=3Us3
...
| 场景 | 推荐话术 |
|------|---------|
| 看到有人抱怨 Claude 贵 | #1、#2、#9、#23 |
| 看到有人问中转站推荐 | #3、#13、#18、#19 |
| 主动在技术社区发帖 | #4、#5、#6、#25、#26 |
| 小红书/即刻种草 | #7、#8、#9、#22、#24 |
| 英文社区 Reddit | #10、#11 |
| QQ/Discord 群内 | #12、#13、#14 |
| B站视频评论区 | #16、#17 |
| 知乎问答 | #18、#19 |
| GitHub / SegmentFault 技术帖 | #20、#21 |
| 即刻 / 微博 超短动态 | #22、#23、#24 |
| 公众号 / 博客文章嵌入 | #25、#26 |
| 竞品话题下拦截 | #27、#28 |
| 独立开发者 / 接外包 | #29 |

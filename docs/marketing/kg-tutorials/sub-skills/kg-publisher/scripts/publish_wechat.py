...
def find_config_path() -> str:
    """向上递归查找 .kg-config.json，最多 6 层；找不到返回空字符串。"""
...
def main():
    ...
    # 读取配置（向上递归查找 .kg-config.json，不依赖固定层级）
    config_path = find_config_path()
    if not config_path:
        print(f"[ERROR] .kg-config.json not found.\n"
              f"  hint: 在技能包根目录 (kg-tutorials/) 下运行: python setup_config.py", file=sys.stderr)
        sys.exit(3)
